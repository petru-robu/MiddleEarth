...
/*
    Reusable turn-based battle loop.
*/
public class BattleEngine {
    ...
    // Runs the full combat encounter for the given quest, then returns the GameState to return to after combat ends
    public static GameState run(Quest quest, GameState returnTo) {
        ...
        if (enemies.isEmpty()) {
            // no enemis, probably a migration issue
        ...
        for (Enemy enemy : enemies) {
            ...
            while (enemy.isAlive() && player.getHealth() > 0) {
                ...
                ui.render("  Your HP:    "
                        + ui.bar(player.getHealth(), player.getCharacterClass().getBaseHealth(), 20,
                                Renderer.Style.GREEN)
                        + "  " + player.getHealth());
                ui.render("  Enemy HP:   " + ui.bar(enemy.getHealth(), enemy.getMaxHealth(), 20, Renderer.Style.RED) +
                        "  " + enemy.getHealth());
                ui.render("");
                
                // get potions and sort them by restore amount
                ...
                if (input.equals("1")) {
                    ...
                    ui.render(Renderer.Style.GREEN + "\nYou strike " + enemy.getName() + " for " + dmg + " damage!"
                            + Renderer.Style.RESET);
                    ...
                    ui.render(Renderer.Style.GREEN + "\nYou drink " + potion.getName() + " and restore " + healed
                            + " HP!" + Renderer.Style.RESET);
            ...
            if (enemy.getLootItem() != null) {
                ...
                if (pick.equals("y") || pick.equals("yes")) {
                    ...
                    ui.render(
                            Renderer.Style.GREEN + loot.getName() + " added to your inventory." + Renderer.Style.RESET);
                }
            }
    ...
    // Helper method to get all potions from inventory
